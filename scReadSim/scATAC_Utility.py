...
def CreateFeatureSets(bedtools_directory, outdirectory, genome_file, ref_peakfile, ref_comple_peakfile, MACS3_peakname_pre):
...
def bam2countmat_countSingle(cells_barcode_file, bed_directory, bed_file, sam_filename, outdirectory, count_mat_filename):
    cells = pd.read_csv(cells_barcode_file, sep="\t")
    cells = cells.values.tolist()
    cells_barcode = [item[0] for item in cells]
    with open("%s/%s" % (outdirectory, count_mat_filename), 'w') as outsfile:
